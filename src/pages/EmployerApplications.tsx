...
const EmployerApplications: React.FC = () => {
  ...
  const handleStatusChange = async (applicationId: string, nextStatus: ApplicationStatus) => {
    try {
      ...
      setStats((prev) => {
        const updated = { ...prev };
        const current = applications.find((a) => a._id === applicationId);
        if (current) {
          updated[current.status] = Math.max((updated[current.status] || 1) - 1, 0);
        }
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-2 text-sm text-gray-700">
                        ...
                        {application.jobId.title} • {application.jobId.company}
                      </div>
